document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('recoverForm').addEventListener('submit', function(e) {
        ...
        const data = { username, email, newPassword, recoverSecurityAnswer };
...
// Fetch and display the stored security question when the user leaves the recoverUsername field
document.getElementById('recoverUsername').addEventListener('blur', function() {
    const username = this.value.trim();
    if (username !== '') {
        fetch(`../php/recover_password.php?username=${encodeURIComponent(username)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    document.getElementById('displaySecurityQuestion').textContent = data.security_question;
                } else {
                    document.getElementById('displaySecurityQuestion').textContent = 'Security question not found.';
                }
            })
            .catch(error => {
                console.error('Error fetching security question:', error);
                document.getElementById('displaySecurityQuestion').textContent = 'Error retrieving security question.';
            });
    } else {
